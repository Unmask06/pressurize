...
class SimulationRequest(BaseModel):
    ...
    p_down_init_psig: float = Field(0.0, description="Initial downstream pressure (psig)", ge=0)
    ...
    opening_mode: str = Field("linear", description="Valve opening mode: linear, exponential, quick_opening, fixed")
    k_curve: float = Field(4.0, description="Curve steepness for exponential/quick_opening")
    dt: float = Field(0.05, description="Time step (s)")
    
    # Composition Mode
    property_mode: str = Field("manual", description="Property mode: manual or composition")
    composition: Optional[str] = Field(None, description="Composition string, e.g., 'Methane=0.9, Ethane=0.1'")
